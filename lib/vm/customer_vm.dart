...
final tempCustomerProvider =
    StateProvider<CustomerModel>((ref) => CustomerModel());
final billCustomerProvider =
    StateProvider<CustomerModel>((ref) => CustomerModel());
...
class CustomerVM extends AsyncNotifier<List<CustomerModel>> {
  ...
  Future<List<CustomerModel>> get(
    ...
    try {
      ...
      if (id != null) {
        final customer =
            (await CustomerRepo.fetchCustomers(where: {"id": id})).first;
  ...
  Future<bool> save(CustomerModel model,
      {TransactionModel transactionModel = const TransactionModel()}) async {
    state = AsyncValue.loading();
    List<dynamic> results = await Future.wait([
      CustomerRepo.addCustomer(model),
      if (transactionModel != TransactionModel())
        TransactionRepo.save(transactionModel.copyWith(
            customerId: model.id ?? (state.value?.length ?? 0) + 1))
    ]);
    final res = results[0];
    if (res != 0) {
      await singleGet(res);
      if (model.id != null) {
        ...
        tempCustomer.state =
            state.value?.where((e) => e.id == tempCustomer.state.id).first ??
                CustomerModel();
      }
    }
    return res != 0;
...
final customerVMProvider =
    AsyncNotifierProvider<CustomerVM, List<CustomerModel>>(CustomerVM.new);
